...
def check_layouts(root: str) -> None:
    ...
    for fn in sorted(os.listdir(lay_dir)):
        ...
        if tdir and lay.get("templates_pending"):
            # Explicit, self-documented "captured + calibrated, templates not
            # harvested yet" state. This is the honest middle of the workflow:
            # the layout's slots/anchor ARE calibrated against real frames, but
            # nobody has cut hero templates yet, so detection correctly reports
            # 'skipped' rather than guessing. A warning (visible, never
            # silent) instead of a failure — the alternative is either a red
            # gate on a legitimately-incomplete pass, or deleting
            # templates_dir, which would then trip the detection_capable check
            # below and lose the record of where templates BELONG.
            warn(f"{fn}: templates_dir '{tdir}' intentionally empty — "
                 "templates_pending is set (calibrated, templates not "
                 "harvested yet; detection honestly reports 'skipped')")
        elif tdir:
