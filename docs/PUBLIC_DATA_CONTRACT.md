...
| `teams` | `[{id, name, code, region, logoUrl, description, website, socials, memberCount, status, organization, aliases, previousNames, sourceAuthority, identityVerifiedAt, rosterSource, rosterVerifiedAt, roster, needsReview, reviewReason, hasLogoCandidate, compositionTrackingPending}]` | **Every team in the registry exports, independent of composition capture** (Phase D2) — a team with zero captured maps still appears, with `compositionTrackingPending:true` instead of being silently omitted. `logoUrl:null` → monogram plate fallback; `description`/`website`/`socials`/`memberCount` are optional FACEIT-sourced facts (Phase D) and are `null` until enriched. `status` is `active`/`inactive`/`historical`/`unsigned`; `previousNames` preserves renamed identities; `needsReview`/`reviewReason` surface an unresolved conflict instead of a silent guess; `roster` is the lineup from the team's most recently dated tracked match (`[]` if none). None of this is ever a substitute for `logoUrl`, which stays human-verified only |
